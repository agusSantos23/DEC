...
const setDB = obj => {
  ...
  switch (obj.constructor.name) { 
    ...
    default: 
      alert("Tipo de objeto no reconocido") 
  }
...
const post = event => {
  ...
  // Si se ha ingresado un código, se está creando un nuevo viaje
    ...
    if (data.precio > 0) {
      ...
      switch (data.selectTypeTrip) {
        ...
        default:
          alert("No se ha encontrado ese tipo de viaje")
          break
      }
    }else{
      alert("El precio tiene que ser un valor superior a 0")
    }
  ...
  // Si se ha seleccionado un cliente y un tipo de viaje, se está creando una nueva reserva
    ...
    if (resultFilter.client && resultFilter.trip) {
      ...
      alert("No se encontró el cliente o el viaje.")
    }
  ...
  if (window.confirm("Estas seguro que quieres eliminar este elemento?")) {
    ...
    switch (tableId) {
      case 'tableClients':
        ...
        const email = rowTable.cells[2].textContent 

        //Comprobamos en la db si el cliente tiene una reserva realizada
        const bookingsFind = db.bookings.some(booking => { 
          const nameClient = `${booking.cliente.nombre} ${booking.cliente.apellido}`
          const client = db.clients.find(c => c.email === email)
          const completName = `${client.nombre} ${client.apellido}`
          return nameClient === completName
        })

        //Si el usaurio tiene reservas a su nombre se le explica la cancelacion del proceso con un alert
        if (bookingsFind) { 
        ...
        db.clients = db.clients.filter(client => client.email !== email)
        break

      case 'tableTrips':
        // Obtenemos el código del viaje desde la celda correspondiente
        const code = rowTable.cells[0].textContent 
        
        //Comprobamos en la db si el cliente tiene una reserva realizada
        const bookingsFindTrip = db.bookings.some(booking => booking.viaje.codigo === code )
        
        //Si el usaurio tiene reservas a su nombre se le explica la cancelacion del proceso con un alert
        if (bookingsFindTrip) { 
          alert("No se puede eliminar el viaje porque tiene reservas asociadas")
          return
        }

        // Filtramos la lista de viajes para eliminar el que coincide con el código
        db.trips = db.trips.filter(trip => trip.codigo !== code)
        break
...
const viewData = () => {
  ...
  //Comprueba si hay algun clientes en la bd
  if (db.clients.length) {
    ...
    //Si no hay ningun cliente se lo comunica al usuario en la tabla de Clientes
    tableClients.innerHTML = "<h3>No se ha encontrado ningun Cliente</h3>"
  )

  //Comprueba si hay algun viajes en la bd
  if (db.trips.length) {
    ...
    //Si no hay ningun viaje se lo comunica al usuario en la tabla de Viaje
    tableTrips.innerHTML = "<h3>No se ha encontrado ningun Viaje</h3>"
  }

  //Comprueba si hay alguna reserva en la bd
  ...
  if (db.bookings.length) {
    ...
    //Si no hay ninguna reserva se lo comunica al usuario en la tabla de reservas
    tableBooking.innerHTML = "<h3>No se ha encontrado ninguna Reserva</h3>"
  }
